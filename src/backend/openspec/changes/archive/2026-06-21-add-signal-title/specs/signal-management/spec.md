...
El sistema MUST permitir que usuarios con rol `Admin` o `Member` creen elementos `Signal` con los campos `Id` (Guid), `Latitud` (float), `Longitud` (float), `Titulo` (cadena), `Descripcion` (cadena), `Fotos` (cadena), `Activo` (boolean), `UserId_Alta` (Guid), `Fecha_Alta` (Datetime), `Fecha_Modificacion` (Datetime), `UserId_Modificacion` (Guid), `Tipo` (entero referenciando `Signal_Type`) y `Tags` (cadena), siendo `Titulo` y `Tipo` obligatorios y validos.

#### Scenario: Alta de signal valida
- **WHEN** un usuario con rol `Admin` o `Member` envia una solicitud de creacion con todos los campos requeridos, incluyendo un `Titulo` informado y un `Tipo` existente
